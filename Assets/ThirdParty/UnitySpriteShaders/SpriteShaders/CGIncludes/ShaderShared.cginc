...
uniform fixed4 _DisplaceColor;
uniform fixed4 _TargetColor;

inline float3 RGBConvertToHSV(float3 c)
{
...
//HSV to RGB
inline float3 HSVConvertToRGB(float3 c)
{
...
#endif
	// fixed3 hsv = RGBConvertToHSV(finalPixel.rgb);
	// hsv.y = 0;
...
inline int ColorLerp(float3 tmp_nowcolor,float3 tmp_FilterfColor)
{
	return saturate(distance(tmp_nowcolor,tmp_FilterfColor) * 1000000000);
}

inline fixed4 calculateVertexColorDisplace(fixed4 color)
{
	color.rgb = (ColorLerp(color.rgb,_TargetColor.rgb) > 0.1) ?   _TargetColor.rgb : color.rgb ;
...
{
	// float3 colorHSV =  RGBConvertToHSV(color.rgb);
	// float3 tarHSV = RGBConvertToHSV(_TargetColor.rgb);
	// colorHSV.z = 0.1;
	//
	// fixed4 vertexColor = color * _Color;
	// #ifdef UNITY_INSTANCING_ENABLED
	// vertexColor *= _RendererColor;
	// #endif
	// float a = ColorLerp(color.rgb,_TargetColor.rgb);
